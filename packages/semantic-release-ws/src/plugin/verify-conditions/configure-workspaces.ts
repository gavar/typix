...
import { VerifyConditionsContext, VerifyReleaseContext, check, hasSemanticReleaseConfig } from "@typix/semantic-release";
...
export async function configureWorkspaces(
  ...
  source: VerifyConditionsContext,
): Promise<Workspace[]> {
  ...
  if (workspaces.length) {
    ...
    for (const workspace of workspaces) {
      ...
      const workspaceConfig = await getConfig(context, options);
      Object.assign(workspace, workspaceConfig);
    }
  }
...
function toWorkspace(this: VerifyReleaseContext, [key, value]: [string, yarn.Workspace]): Workspace {
...
async function checkWorkspace(workspace: Workspace, { logger }: VerifyConditionsContext): Promise<boolean> {
  ...
  const violation = await check(workspace, checkSemanticReleaseConfig);
...
async function checkSemanticReleaseConfig(workspace: Workspace): Promise<true | string> {
  return (
    await hasSemanticReleaseConfig(workspace.cwd)
